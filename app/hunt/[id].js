...
const ScrollIcon = ({ size = 18, color = C.gold }) => (
  <View style={{ width: size, height: size * 1.2, borderRadius: 4, borderWidth: 1.5, borderColor: color, alignItems: 'center', justifyContent: 'center', gap: 3 }}>
    {[0.7, 0.5, 0.35].map((w, i) => (
      <View key={i} style={{ width: size * w, height: 1.5, backgroundColor: color, opacity: 1 - i * 0.25 }} />
    ))}
  </View>
);
...
const GroupIcon = ({ size = 14, color = C.accent }) => (
  <View style={{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }}>
    <View style={{ width: size * 0.42, height: size * 0.42, borderRadius: size * 0.21, borderWidth: 1.5, borderColor: color, position: 'absolute', left: 0 }} />
    <View style={{ width: size * 0.42, height: size * 0.42, borderRadius: size * 0.21, borderWidth: 1.5, borderColor: color, position: 'absolute', left: size * 0.24 }} />
    <View style={{ width: size * 0.55, height: size * 0.28, borderTopLeftRadius: size * 0.28, borderTopRightRadius: size * 0.28, borderWidth: 1.5, borderColor: color, borderBottomWidth: 0, position: 'absolute', bottom: 0, left: 0 }} />
    <View style={{ width: size * 0.55, height: size * 0.28, borderTopLeftRadius: size * 0.28, borderTopRightRadius: size * 0.28, borderWidth: 1.5, borderColor: color, borderBottomWidth: 0, position: 'absolute', bottom: 0, left: size * 0.2 }} />
  </View>
);

const GiftIcon = ({ size = 14, color = C.gold }) => (
  <View style={{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }}>
    <View style={{ width: size * 0.85, height: size * 0.6, borderRadius: 2, borderWidth: 1.5, borderColor: color, position: 'absolute', bottom: 0 }} />
    <View style={{ width: size * 0.85, height: size * 0.22, borderRadius: 1, borderWidth: 1.5, borderColor: color, position: 'absolute', top: size * 0.25 }} />
    <View style={{ width: 1.5, height: size * 0.6, backgroundColor: color, position: 'absolute', bottom: 0 }} />
    <View style={{ width: size * 0.3, height: size * 0.22, borderTopLeftRadius: size * 0.15, borderTopRightRadius: size * 0.15, borderWidth: 1.5, borderColor: color, borderBottomWidth: 0, position: 'absolute', top: 0, left: size * 0.15 }} />
    <View style={{ width: size * 0.3, height: size * 0.22, borderTopLeftRadius: size * 0.15, borderTopRightRadius: size * 0.15, borderWidth: 1.5, borderColor: color, borderBottomWidth: 0, position: 'absolute', top: 0, right: size * 0.15 }} />
  </View>
);
      ...
      <Animated.View style={[styles.header, { opacity: headerFade }]}>
        ...
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerCenter}>
            <ScrollIcon size={16} color={C.gold} />
            <Text style={styles.headerTitle}>Détail</Text>
          </View>
          {hunt && (
            <View style={styles.huntIdBadge}>
              <Text style={styles.huntIdText}>#{String(hunt.id).padStart(3, '0')}</Text>
            </View>
          )}
        </View>
      </Animated.View>
      ...
      >
        {loading ? (
          ...
          <>
              ...
              <View style={styles.heroInner}>
                ...
                <View style={styles.heroBadgeRow}>
                  <View style={styles.diffRow}>
                    {[1, 2, 3].map(d => (
                      <View key={d} style={[styles.diffDot, { backgroundColor: d <= difficulty ? C.gold : C.border }]} />
                    ))}
                  </View>
                  {hunt.organizer?.username && (
                    <View style={styles.organizerBadge}>
                      <UserIcon size={11} color={C.goldDim} />
                      <Text style={styles.organizerText}>{hunt.organizer.username}</Text>
                    </View>
                  )}
                </View>
                ...
                <View style={styles.heroStats}>
                  <View style={styles.heroStat}>
                    ...
                    <Text style={styles.heroStatText}>
                      {hunt.participants?.length ?? 0} participant{hunt.participants?.length !== 1 ? 's' : ''}
                    </Text>
                  </View>
                  ...
                  <View style={styles.heroStat}>
                    ...
                    <Text style={styles.heroStatText}>
                      {hunt.rewards?.reduce((acc, r) => acc + (r.crownAmount || 0), 0) ?? 0} couronnes
                    </Text>
                  </View>
                </View>
              </View>
            ...
            {/* ── DESCRIPTION ── */}
            <Section icon={<ScrollIcon size={13} color={C.goldDim} />} title="DESCRIPTION" delay={200}>
              <Text style={styles.descText}>
                {hunt.description || 'Aucune description disponible pour cette chasse.'}
              </Text>
            </Section>

            {/* ── PARTICIPANTS ── */}
            <Section icon={<GroupIcon size={13} color={C.accent} />} title="PARTICIPANTS" delay={300}>
              {hunt.participants?.length > 0 ? (
                ...
                <Text style={styles.emptyRowText}>Aucun participant pour l'instant</Text>
              )}
            </Section>

            {/* ── RÉCOMPENSES ── */}
            <Section icon={<GiftIcon size={13} color={C.gold} />} title="RÉCOMPENSES" delay={400}>
              {hunt.rewards?.length > 0 ? (
                ...
                <Text style={styles.emptyRowText}>Aucune récompense définie</Text>
              )}
            </Section>
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  huntIdText: { fontSize: 11, color: C.goldDim, fontFamily: 'monospace', letterSpacing: 1 },
  ...
  heroStatSep: { width: 3, height: 3, borderRadius: 1.5, backgroundColor: C.textFaint },
  ...
  sectionTitle: {
    ...
    color: C.textMuted, fontFamily: 'monospace', letterSpacing: 2,
  },
  ...
  pill: {
    ...
    borderWidth: 1, borderColor: C.border,
    paddingRight: 12, paddingLeft: 4, paddingVertical: 5,
  },
  pillAvatar: {
  ...
  rewardLabel: { fontSize: 11, color: C.textMuted, fontFamily: 'monospace', letterSpacing: 0.5 },
  ...
  // Skeleton
  skeleton: {
    height: 18, backgroundColor: C.surface,
